...
const Artists: React.FC = async () => {
  ...
  artistsData = artistsData.sort((a: ArtistWithEvents, b: ArtistWithEvents) => {
  ...
  const artists: ArtistWithEvents[] = artistsData.map((a: ArtistWithEvents) => {
    ...
    if (
      !a.artist.imageUrl ||
      a.artist.imageUrl === "ZgotmplZ" ||
      a.artist.imageUrl === "NULL"
    ) {
      return {
        ...a,
        artist: { ...a.artist, imageUrl: "/default-artist-image.png" },
      };
    } else {
      return a;
    }
  ...
  if (usePlaceHolders !== "false") {
    const imagesWithPlaceholders = await Promise.all(
      artists.map(async (artist: ArtistWithEvents) => {
        if (
          ...
          artist.artist.imageUrl === "ZgotmplZ" ||
          artist.artist.imageUrl === "NULL"
        ) {
          return {
            src: "/default-artist-image.png",
            placeholder: "/default-artist-image.png",
          };
        }
    ...
    const imagesWithOutPlaceholders: ImageWithPlaceholder[] = artists.map(
  ...
  return (
    <div className="artists-page-wrapper">
      ...
      <div className="artists-grid">
        {artistsWithPlaceholders.map(
          (a: ArtistWithEventsAndPlaceholderImage) => (
            <Link
              key={a.artist.id}
              href={`/kuenstlerinnen/${a.artist.code}`}
              className="artist-link"
            >
              <div
                key={a.artist.id}
                className={`artist-card ${a.artist.attributes?.astroprogramm ? "astroprogramm" : ""}`}
              >
                <Image
